setwd("/scATAC-machineLearning-benchmarking/intra-Buenrostro2018/bin")

label<-read.csv('../input/metadata_buenrostro2018_sorted.tsv',sep = "\t",header = TRUE)
...
saveRDS(data,file="../output/buenrostro2018-snap-full.rds")

# label<-label[order(label[,1]),]

# write.table(label,file="../input/metadata_buenrostro2018_sorted.tsv",quote=FALSE,sep="\t",row.names = FALSE)
