...
def run_DT(trainDataPath,testDataPath, trainLabelsPath,testLabelsPath, CV_RDataPath, OutputDir):
    '''
    run baseline classifier: DT
    Wrapper script to run an DT classifier on a benchmark dataset with 5-fold cross validation,
...
#     print(len(tr_time))
    ...
    truelab.to_csv(str(OutputDir / Path("DT_true.csv")),
                   index = False)
    pred.to_csv(str(OutputDir / Path("DT_pred.csv")),
                index = False)
    tr_time.to_csv(str(OutputDir / Path("DT_training_time.csv")),
                   index = False)
    ts_time.to_csv(str(OutputDir / Path("DT_test_time.csv")),
                   index = False)


run_DT("../input/10xpbmc5k-snap-full_v1Train.rds","../input/10xpbmc5k-snap-full_nextgemTest.rds", "../input/pbmc_5k_atac_label_v1.txt","../input/pbmc_5k_atac_label_nextgem.txt", "../tmp/CV_folds_v1Train_nextgemTest.RData", "../output")
